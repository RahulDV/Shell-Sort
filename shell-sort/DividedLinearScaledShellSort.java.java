...
public class ShellSort {
	...
	public static void main(String[] args) throws InterruptedException{
		...
		for(int j=1;j<=10;j++){
			...
			double logValue=0;
			for(;n>0;n=n-1){
				logValue = logValue + Math.log10(n);
			}
			logValue = logValue/Math.log10(2);
			for (int gapSequenceCode = 1; gapSequenceCode <= 4; gapSequenceCode++) {
				List<Integer> hvalues = getGapSequence(100000*j, gapSequenceCode);
				int total = 0;
				for (int i = 0; i < 10; i++) {
					total = total + shellSort(createRandomArray(100000*j),hvalues);
				}
				double avNOC = total / 10;
				avNOC = avNOC/logValue;
				System.out.println("average number of comparisons for input "+ 100000 * j + " and gap sequence code "+gapSequenceCode+ " is = " + avNOC);
			}
		}
